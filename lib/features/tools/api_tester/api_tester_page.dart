  ...
  void _showHeadersDialog() {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Response Headers', style: TextStyle(color: AppColors.ink, fontSize: 14)),
        content: SizedBox(
          width: double.maxFinite,
          child: ConsoleOutput(
            content: _responseHeaders!.entries.map((e) => '${e.key}: ${e.value}').join('\n'),
            label: 'headers',
            maxHeight: 300,
          ),
        ),
          ...
          Row(
            children: [
              ...
              if (_responseHeaders != null) ...[
                const SizedBox(width: 10),
                GestureDetector(
                  onTap: () => _showHeadersDialog(),
                  child: Text('${_responseHeaders!.length} headers',
                      style: const TextStyle(color: AppColors.cyan, fontSize: 11, decoration: TextDecoration.underline)),
                ),
              ],
            ],
          ),
          const SizedBox(height: 10),
          ConsoleOutput(content: _responseBody ?? '', label: 'response.json', maxHeight: 320),
        ],
      ],
    );
  }
