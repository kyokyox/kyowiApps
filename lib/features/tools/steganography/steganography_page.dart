...
class _SteganographyPageState extends State<SteganographyPage> {
  ...
  Future<void> _decode() async {
    ...
    try {
      ...
      if (text == null) {
        setState(() => _error =
            'Gak ketemu pesan tersembunyi. Gambar ini bukan hasil steganography Kyowi, atau udah kekompresi (misal disave ulang sebagai JPG).');
      } else {
  ...
  Future<void> _saveToGallery() async {
    ...
    try {
      ...
      await file.delete();
      setState(() => _successMessage = '✓ Tersimpan di Galeri (album "Kyowi")');
    } catch (e) {
    ...
    return Column(
      ...
      children: [
        ...
        if (_decodedText != null) ...[
          ...
          const SizedBox(height: 8),
          ConsoleOutput(content: _decodedText!, label: 'decode.sh'),
        ],
      ],
    );
  }
